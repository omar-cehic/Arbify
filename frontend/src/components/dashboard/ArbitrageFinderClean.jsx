...
const ArbitrageFinder = () => {
  ...
  // Function to fetch arbitrage opportunities from SGO API
  ...
  const loadOpportunities = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      try {
        ...
        console.log('SGO API: Loaded', sgoData.arbitrage_opportunities?.length || 0, 'opportunities');
        ...
        console.error('SGO API failed, trying legacy API:', sgoError);
      }
  ...
  const saveOpportunity = async (opportunity) => {
    ...
    try {
      const arbitrageData = {
        event_id: opportunity.event_id || opportunity.id,
        ...
        bookmakers_involved: opportunity.bookmakers_involved,
            ...
            <div className="grid gap-6">
              {filteredOpportunities.map((opportunity, index) => (
                <ArbitrageCard
                  ...
                  userTier={user?.subscription?.plan?.name || 'basic'}
                />
              ))}
            </div>
          ) : (
